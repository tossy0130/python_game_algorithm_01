...
def draw_table():
    ...
    cvs.create_text(500, 40, text=str(point_com)+" - "+str(point_you), font=FNT, fill="white")
    if proc==0:
    ...
    if proc==2:
        if point_you==POINT_WIN:
            cvs.create_text(1000-tmr*10, 300, text="YOU WIN!", font=FNT, fill="cyan")
        else:
...
def smasher_com():
    ...
    if get_dis(com_x,com_y,pu_x,pu_y)<50*50:
        ...
        if com_y<pu_y-dots: com_y += dots
        if com_y>pu_y+dots: com_y -= dots
        if com_x<pu_x-dots: com_x += dots
        if com_x>pu_x+dots: com_x -= dots
    else:
...
def puck():
    ...
    if pu_x>1000-r and pu_vx>0:
        pu_vx = -pu_vx
    if pu_y<0: pu_y = 0
    if pu_y>600: pu_y = 600
    if pu_x<0: pu_x = 0
    if pu_x>1000: pu_x = 1000
    pu_vx = pu_vx*0.95
    pu_vy = pu_vy*0.95
